...
# Import dataset
#df = pd.read_csv('donnees_essentiel_ecobalyse_train.csv')
df = pd.read_csv('C:/Users/ruxan/OneDrive/Desktop/M2_SEP/Projet digital/PD/donnees_essentiel_ecobalyse.csv')
...
df_summary_product_ = df.groupby(['product']).agg({
    ...
    'impact_ecs': [
        'mean', 'min', 'max',  # Basic stats
        lambda x: x.quantile(0.25),  # Q1
        lambda x: x.quantile(0.75)   # Q3
    ]
})

# Rename the custom quartile aggregations for clarity
df_summary_product_.columns = [
    'mass_mean', 'mass_min', 'mass_max', 
    'impact_mean', 'impact_min', 'impact_max', 'impact_q1', 'impact_q3'
]
...
# Step 1: Group data and calculate statistics
...
#z_value = norm.ppf(1 - (1 - confidence_level) / 2)  # Z-value for 95% CI

ci_lower = df.groupby('product')['impact_ecs'].min()# means - z_value * (std_devs / np.sqrt(counts))
ci_upper = df.groupby('product')['impact_ecs'].max()# means + z_value * (std_devs / np.sqrt(counts))
...
for i, (effect, lower, upper) in enumerate(zip(effects, ci_lower, ci_upper)):
    ax.plot([lower, upper], [i, i], color='black', lw=1)  # Horizontal line for CI
